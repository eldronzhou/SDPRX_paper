...
python ../../../software/PRScsx/PRScsx.py \
  ...
  --sst_file=/ysm-gpfs/pi/zhao-data/gz222/T2D/summ_stats/PRS_cs.txt,../../summary_stat/EAS/PRS_cs.txt \
  --n_gwas=156109,191764 \
  --pop=${pop1},${pop2} \
  --out_dir=./ \
  --out_name=PRS_cs \
  --chrom=${i}
...
do
    mkdir -p ${phi}/
    python ../../../software/PRScsx/PRScsx.py --ref_dir=/ysm-gpfs/pi/zhao/gz222/UKB_real/ref/PRS_CS/ --bim_prefix=../../../genotype/EAS/Ukb_imp_v2 --sst_file=/ysm-gpfs/pi/zhao-data/gz222/T2D/summ_stats/PRS_cs.txt,../../summary_stat/EAS/PRS_cs.txt --n_gwas=156109,191764 --pop=${pop1},${pop2} --out_dir=${phi}/ --out_name=PRS_cs --phi=${phi} --chrom=${i}
