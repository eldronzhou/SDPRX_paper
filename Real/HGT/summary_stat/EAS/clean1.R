...
system("~/software/ldsc/ldsc.py --h2 clean.sumstats.gz --ref-ld-chr /ysm-gpfs/pi/zhao/gz222/prs_comparison/LDSC/eas_ldscores/ --out ./clean_h2 --w-ld-chr /ysm-gpfs/pi/zhao/gz222/prs_comparison/LDSC/eas_ldscores/")
...
a$N = 159095
write.table(a[,c(1,5,2,3,4,8,9,7,11)], file="ldpred2.txt", append=F, sep="\t", quote=F, row.names=F, col.names=T)

system("rm -rf clean1.txt")

system("source activate python2")

system("popcorn fit -v 0 --cfile ../../../software/Popcorn/ref/EUR_EAS_all_gen_eff.cscore --gen_effect --sfile1 ../EUR/XPASS.txt --sfile2 XPASS.txt ./popcorn.txt")
